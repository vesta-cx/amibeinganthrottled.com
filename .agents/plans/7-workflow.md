...
A single GitHub Actions workflow file that runs `generate-copy.js` every Monday at 00:00 UTC, commits the new copy file to `main` on success, and fails visibly on any error.
...
- [ ] `.github/workflows/generate-copy.yml` present with cron `0 0 * * 1`
