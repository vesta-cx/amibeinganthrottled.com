...
export const buildEnglishPrompt = (dedup: string[] = []): string => {
	const dedupBlock =
		dedup.length > 0
			? `\nDo not repeat or closely echo any of these strings from the previous week:\n${dedup.map((s) => `- ${s}`).join('\n')}\n`
			: ''
...
their usage limits faster than normal. Commiserate. Mild outrage at the \
absurdity of paying for a subscription that throttles you during the hours \
you'd most want to use it. Gently suggest they step away — they can't use \
Claude properly right now anyway, so they might as well exist in the physical \
world for a bit. Resigned, not angry. Example register: \
