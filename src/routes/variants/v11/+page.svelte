<script lang="ts">
	...
	const darkColors: Record<ThrottleState, { primary: string; glow: string; bg: string; cardBg: string; text: string; sub: string }> = {
		throttled: { primary: '#f38ba8', glow: 'rgba(243,139,168,0.25)', bg: '#11111b', cardBg: '#1e1e2e', text: '#cdd6f4', sub: '#a6adc8' },
		clear: { primary: '#a6e3a1', glow: 'rgba(166,227,161,0.25)', bg: '#11111b', cardBg: '#1e1e2e', text: '#cdd6f4', sub: '#a6adc8' },
		weekend: { primary: '#cba6f7', glow: 'rgba(203,166,247,0.25)', bg: '#11111b', cardBg: '#1e1e2e', text: '#cdd6f4', sub: '#a6adc8' },
	};
	...
	$effect(() => {
		...
		const draw = () => {
			...
			// Plasma parameters per state
			const speed = state === 'throttled' ? 2.0 : state === 'weekend' ? 0.4 : 1.0;
			const complexity = state === 'throttled' ? 6 : state === 'weekend' ? 3 : 4;
			...
			for (let row = 0; row < rows; row++) {
				for (let col = 0; col < cols; col++) {
					...
					// Pointer warp — displace coordinates near cursor
					const dx = nx - mouseX;
					const dy = ny - mouseY;
					const pointerDist = Math.sqrt(dx * dx + dy * dy);
					const warp = Math.max(0, 1 - pointerDist / 0.25) * 0.4;
					const wx = nx + (pointerDist > 0.01 ? (dx / pointerDist) * warp : 0);
					const wy = ny + (pointerDist > 0.01 ? (dy / pointerDist) * warp : 0);

					// Classic plasma: sum of sine functions
					let v = 0;
					v += Math.sin(wx * complexity + t * speed);
					v += Math.sin(wy * complexity * 1.3 + t * speed * 0.7);
					v += Math.sin((wx + wy) * complexity * 0.7 + t * speed * 1.3);
					v += Math.sin(Math.sqrt(wx * wx * 4 + wy * wy * 4) * complexity + t * speed * 0.5);

					// Additional interference
					v += Math.sin(wx * complexity * 2.1 + wy * complexity * 1.7 + t * speed * 0.8) * 0.5;
					v += Math.cos(wy * complexity * 2.3 - wx * complexity * 1.1 + t * speed * 1.1) * 0.5;

					// Normalize to 0..1
					const intensity = (Math.sin(v * Math.PI * 0.25) + 1) * 0.5;
...
<div class="page" style="background: {colors.bg}; font-family: 'Lexend Variable', sans-serif;">
	<div class="card" style="background: {colors.cardBg}; box-shadow: 0 0 50px {colors.glow}, 0 4px 30px rgba(0,0,0,0.3);">
