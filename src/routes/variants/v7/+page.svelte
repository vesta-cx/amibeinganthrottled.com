<script lang="ts">
	...
	const lightColors: Record<ThrottleState, { primary: string; glow: string; bg: string; cardBg: string; text: string; sub: string }> = {
		throttled: { primary: '#d20f39', glow: 'rgba(210,15,57,0.2)', bg: '#eff1f5', cardBg: '#ffffff', text: '#4c4f69', sub: '#6c6f85' },
		clear: { primary: '#40a02b', glow: 'rgba(64,160,43,0.2)', bg: '#eff1f5', cardBg: '#ffffff', text: '#4c4f69', sub: '#6c6f85' },
		weekend: { primary: '#8839ef', glow: 'rgba(136,57,239,0.2)', bg: '#eff1f5', cardBg: '#ffffff', text: '#4c4f69', sub: '#6c6f85' },
	};
	...
	$effect(() => {
		...
		const draw = () => {
			...
			const speedMul = state === 'throttled' ? 1.8 : state === 'weekend' ? 0.5 : 1.0;

			const blobs = [
				{ x: 0.3 + Math.sin(t * 0.7) * 0.2, y: 0.4 + Math.cos(t * 0.5) * 0.25, r: 0.18 },
				{ x: 0.7 + Math.cos(t * 0.6) * 0.15, y: 0.6 + Math.sin(t * 0.8) * 0.2, r: 0.15 },
				{ x: 0.5 + Math.sin(t * 0.4 + 1) * 0.25, y: 0.3 + Math.cos(t * 0.3) * 0.25, r: 0.2 },
				{ x: 0.4 + Math.cos(t * 0.9 + 2) * 0.2, y: 0.7 + Math.sin(t * 0.6 + 1) * 0.2, r: 0.14 },
				{ x: 0.6 + Math.sin(t * 0.5 + 3) * 0.22, y: 0.5 + Math.cos(t * 0.7 + 2) * 0.22, r: 0.16 },
				{ x: mouseX, y: mouseY, r: 0.24 },
			];
...
<div class="page" style="background: {colors.bg}; font-family: 'Quicksand Variable', sans-serif;">
	<div class="card" style="background: {colors.cardBg}; box-shadow: 0 0 60px {colors.glow}, 0 4px 40px rgba(0,0,0,0.4);">
